...
def test_roundtrip_sorts_links_and_groups(paths):
    save_state(paths, {"claude": ToolState(groups=["design", "coding"], links=["b", "a"])})
    st = load_state(paths)
...
def test_backup_and_latest(paths):
    p1 = backup(paths, "hermes", ToolState(groups=[], links=["x"]))
    p2 = backup(paths, "hermes", ToolState(groups=["research"], links=["y"]))
    assert p1 != p2 and p1.exists() and p2.exists()
    snap = latest_backup(paths, "hermes")
    assert snap.groups == ["research"] and snap.links == ["y"]
